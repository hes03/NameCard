...
//화면이 다시 그려지면 함수도 새로 만들어진다. - 비효율적(부분갱신이나 부분처리 가능)
const ImageFileInput = ({name, imageUploader, onFileChange}) => {
  ...
  // file명을 선택하고 업로드 중일 때 상태관리
  // const a = 10 // a에 값이 변하더라도 새로 그려지지 않음
  // 일반변수 a는 값이 아무리 변경되더라도 다시 그려지지 않는다.
  // 함수가 새로 생성되지 않는다 | 새로 생성됨 -> useEffect의 의존성배열 관련
  // useEffect 내부에 실행문이 실행이 된다 | 실행이 되지 않는다
  const [loading, setLoading] = useState(false)
  // input type file에 변화가 생기면 호출되는 함수이다.
  const handleChange = async(event) => {
    //로딩 애니메이션 효과 발동 -> 상태값을 false -> true 변경해줌
    setLoading(true)
